...
function showSocialRankWindow ()
	...
	if gWindow["socialRankSelection"] then
		...
		gLabel["socialRankInfo"] = guiCreateLabel(175,30,227,112,"In diesem Menue kannst du deine frei-\ngeschalteten sozialen Staten akti-\nvieren, damit dieser in der Spielerliste\nsichtbar wird.\nAusserdem wird dir angezeigt, welchen\nStatus du als naechstes erreichen\nkannst bzw. was dafuer noetig ist,\nden jeweiligen Status zu erreichen.",false,gWindow["socialRankSelection"])
		...
		gLabel["socialRankReqs"] = guiCreateLabel(193,148,59,15,"Benoetigt:",false,gWindow["socialRankSelection"])
			...
			if state == "up" then
				...
				if selectedText then
					if selectedTextAvailable == "[x]" then
						setElementData ( lp, "socialState", selectedText, true )
