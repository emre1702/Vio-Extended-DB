...
function changeFireMode ()
	...
	if fireModeWeapons[getPedWeapon ( lp )] and not getPedOccupiedVehicle ( lp ) then
		if firestate == "auto" then
			if ( vioGetElementData ( lp, "fraktion" ) == 8 and vioGetElementData ( lp, "rang" ) >= 1 ) or vioGetElementData ( lp, "fraktion" ) ~= 8 then
				firestate = "burst"
			elseif vioGetElementData ( lp, "fraktion" ) == 8 and vioGetElementData ( lp, "rang" ) == 0 then
				firestate = "semi"
			end
		...
		-- firestate == "semi"
			if vioGetElementData ( lp, "fraktion" ) == 8 and vioGetElementData ( lp, "rang" ) >= 1 then
				firestate = "burst"
			elseif vioGetElementData ( lp, "fraktion" ) ~= 8 then
				firestate = "semi"
			end
			firestate = "auto"
		end
...
function bindFireModeChangeKey ()
	...
	if vioGetElementData ( lp, "fraktion" ) == 8 then
		if vioGetElementData ( lp, "rang" ) > 1 then
			firestate = "auto"
		elseif vioGetElementData ( lp, "rang" ) == 1 then
			firestate = "burst"
		else
			firestate = "semi"
		end
