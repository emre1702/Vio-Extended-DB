		...
		if isOnDuty(player) then
			...
			if action == "ex" then
				...
				elseif slot == 2 then
					...
					triggerClientEvent ( player, "sec_gun_give", getRootElement(), gun2, ammo2 )
				else
					vioSetElementData ( player, "gunboxc", "0|0" )
					giveWeapon ( player, gun3, ammo3, true )
					triggerClientEvent ( player, "sec_gun_give", getRootElement(), gun3, ammo3 )
				end
			elseif action == "in" then
				...
				for i = 1, 12 do
					vioSetElementData ( player, "weaponInSlot"..i, 0 )
					vioSetElementData ( player, "weaponInSlot"..i.."Ammo", 0 )
				end
				takeWeapon ( player, curgun )
				if vioGetElementData ( player, "playingtime" ) <= 180 and curgun >= 9 then
					banVioPlayerServer ( player, "Waffencheat" )
				end
			end
