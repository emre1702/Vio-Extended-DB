			...
			function ( btn, state )
				if state == "up" then
					...
					if row then
						...
						if gebot then
							if getElementData ( lp, "bankmoney" ) >= gebot then
								local count = _G[row.."DataCount"]
								local status = true
								if gColumn["secText"] then
									if secColumn == "Model" then
										if premiumCars [ counts ] and not getElementData ( lp, "premium" ) then
											outputChatBox ( "Du kannst keine Premiumfahrzeuge kaufen, wenn du kein Premiumbenutzer bist!", 125, 0, 0 )
											status = false
										end
									end
								end
