...
function showInventory_func ()
	...
	if not isCursorShowing () and not getElementData ( lp, "ElementClicked" ) and not tutorial and not isPedDead ( lp ) then
		...
		if gWindow["inventory"] then
			...
			guiWindowSetMovable(gWindow["inventory"],false)
			guiWindowSetSizable(gWindow["inventory"],false)
			
			gGrid["itemList"] = guiCreateGridList(0.0295*325,0.1007*288,0.4721*325+50,0.8715*288,false,gWindow["inventory"])
			...
			gImage["itemImage"] = guiCreateStaticImage(0.5049*325+50,0.1111*288,0.1639*325,0.1736*288,"images/black.bmp",false,gWindow["inventory"])
			
			gLabel["itemName"] = guiCreateLabel(0.6721*325+50,0.1736*288,0.3049*325,0.1146*288,"benzinkannister",false,gWindow["inventory"])
			...
			gLabel["itemInfo"] = guiCreateLabel(0.5049*325+50,0.3125*288,0.4689*325,0.3507*288,"Ein Kanister erlaubt\nes dir, mit /fill ein beliebi-\nges Fahrzeug mit 15 \nLitern Benzin zu fuellen.",false,gWindow["inventory"])
			...
			gButton["itemUse"] = guiCreateButton(0.5049*325+50,0.6806*288,0.2164*325,0.1319*288,"Benutzen",false,gWindow["inventory"])
			guiSetAlpha(gButton["itemUse"],1)
			gButton["itemThrow"] = guiCreateButton(0.7344*325+50,0.6806*288,0.2164*325,0.1319*288,"Wegwerfen",false,gWindow["inventory"])
			guiSetAlpha(gButton["itemThrow"],1)
			gButton["itemClose"] = guiCreateButton(0.623*325+50,0.835*288,0.2164*325,0.1319*288,"Schliessen",false,gWindow["inventory"])
...
function useCurrentItem ()
	...
	elseif itemCommand[selectedText] then
		if itemCommand[selectedText] == "eat" then
			for i = 1, 3 do
				if foodName[getElementData ( lp, "food"..i )] == selectedText then
...
function newSelection ()
	...
	if selectedText then
		if validItems[selectedText] then
			...
			gImage["itemImage"] = guiCreateStaticImage(0.5049*325+50,0.1111*288,0.1639*325,0.1736*288,"images/inventory/"..itemImage[selectedText],false,gWindow["inventory"])
			guiSetAlpha ( gImage["itemImage"], 1 )
		elseif placeAbleObjects[selectedText] then
			local addtext = ""
			if placeAbleObjectDesc[getElementData ( lp, "object" )] then
				addtext = addtext..placeAbleObjectDesc[getElementData ( lp, "object" )]
			end
			if gImage["itemImage"] then
				destroyElement ( gImage["itemImage"] )
			end
			...
			gImage["itemImage"] = guiCreateStaticImage(0.5049*325+50,0.1111*288,0.1639*325,0.1736*288,path,false,gWindow["inventory"])
...
function fillWithItems ( grid, columnName, columnCount )
	...
	-- Suessigkeitener --
	if getElementData ( lp, "easterEggs" ) >= 1 then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, "Kuerbis", false, false )
		if getElementData ( lp, "easterEggs" ) == 1 then value = "" else value = getElementData ( lp, "easterEggs" ) end
		guiGridListSetItemText ( grid, row, columnCount, value, false, false )
	end
	-- placeAbleObjects --
	if getElementData ( lp, "object" ) > 0 then
		local row = guiGridListAddRow ( grid )
		local objectName = placeAbleObjectNames[getElementData ( lp, "object" )]
	...
	-- Handheld --
	if getElementData ( lp, "gameboy" ) >= 1 then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, "Handheld", false, false )
		if getElementData ( lp, "gameboy" ) == 1 then value = "" else value = getElementData ( lp, "gameboy" ) end
		guiGridListSetItemText ( grid, row, columnCount, value, false, false )
	end
	-- Fernglas
	if getElementData ( lp, "fglass" ) then
		local row = guiGridListAddRow ( grid )
	...
	-- Schach --
	if getElementData ( lp, "chess" ) then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, "Schach", false, false )
		guiGridListSetItemText ( grid, row, columnCount, "", false, false )
	end
	-- Geschenk --
	if getElementData ( lp, "presents" ) then
		if getElementData ( lp, "presents" ) > 0 then
			local row = guiGridListAddRow ( grid )
			guiGridListSetItemText ( grid, row, columnName, "Geschenk", false, false )
			guiGridListSetItemText ( grid, row, columnCount, tostring ( getElementData ( lp, "presents" ) ), false, false )
		end
	end
	-- Notebook --
	if getElementData ( lp, "fruitNotebook" ) >= 1 then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, "Notebook", false, false )
		if getElementData ( lp, "fruitNotebook" ) == 1 then value = "" else value = getElementData ( lp, "fruitNotebook" ) end
		guiGridListSetItemText ( grid, row, columnCount, value, false, false )
	end
	-- Zeitung --
	if getElementData ( lp, "newspaper" ) then
		local row = guiGridListAddRow ( grid )
	...
	-- Chips --
	local chips = tonumber ( getElementData ( lp, "casinoChips" ) )
	if chips >= 1 then
		local row = guiGridListAddRow ( grid )
	...
	-- Hufeisen --
	local horseshoes = tonumber ( getElementData ( lp, "totalHorseShoes" ) )
	...
	-- Orden 1-3 --
	local orden_1 = getElementData ( lp, "armyperm6" )
	local orden_2 = getElementData ( lp, "armyperm7" )
	local orden_3 = getElementData ( lp, "armyperm8" )
	...
	-- Überschrift --
	for i = 1, 3 do
		if getElementData ( lp, "food"..i ) >= 1 then
			local row = guiGridListAddRow ( grid )
	...
	-- Foodslot 1 --
	local food = tonumber ( getElementData ( lp, "food1" ) )
	if food >= 1 then
		local row = guiGridListAddRow ( grid )
	...
	-- Foodslot 2 --
	local food = tonumber ( getElementData ( lp, "food2" ) )
	if food >= 1 then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, foodName[food], false, false )
		guiGridListSetItemText ( grid, row, columnCount, "", false, false )
	...
	-- Foodslot 3 --
	local food = tonumber ( getElementData ( lp, "food3" ) )
	if food >= 1 then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, foodName[food], false, false )
		guiGridListSetItemText ( grid, row, columnCount, "", false, false )
	...
	-- Angelausrüstung --
	if getElementData ( lp, "fishingPole" ) then
		local row = guiGridListAddRow ( grid )
		...
		if getElementData ( lp, "fishingHooks" ) > 0 then
			local row = guiGridListAddRow ( grid )
			guiGridListSetItemText ( grid, row, columnName, "Haken", false, false )
			guiGridListSetItemText ( grid, row, columnCount, tostring ( getElementData ( lp, "fishingHooks" ) ), false, false )
		end
		if getElementData ( lp, "fishingWorms" ) > 0 then
			local row = guiGridListAddRow ( grid )
			guiGridListSetItemText ( grid, row, columnName, "Koeder", false, false )
			guiGridListSetItemText ( grid, row, columnCount, tostring ( getElementData ( lp, "fishingWorms" ) ), false, false )
		end
		local typ, name, weight
		for i = 65, 67 do
			typ = string.char ( i )
			name = getElementData ( player, "fishingFish"..typ.."Typ" )
			if tonumber ( name ) then
				if tonumber ( name )>= 1 then
					weight = getElementData ( player, "fishingFish"..typ.."Weight" )
					local row = guiGridListAddRow ( grid )
	...
	local hasSpecialAmmo = false
	for i = 1, 100 do
		if not specialAmmoName[i] then
			break
		else
			if vioGetElementData ( lp, "ammoTyp"..i ) then
				if vioGetElementData ( lp, "ammoTyp"..i ) > 0 then
					hasSpecialAmmo = true
					break
				end
			end
	...
	if hasSpecialAmmo then
		local row = guiGridListAddRow ( grid )
		guiGridListSetItemText ( grid, row, columnName, "Spezialmunnition", true, false )
		for i = 1, 100 do
			if not specialAmmoName[i] then
				break
			else
				if vioGetElementData ( lp, "ammoTyp"..i ) then
					if vioGetElementData ( lp, "ammoTyp"..i ) > 0 then
						local row = guiGridListAddRow ( grid )
						guiGridListSetItemText ( grid, row, columnName, specialAmmoName[i], false, false )
						guiGridListSetItemText ( grid, row, columnCount, tostring ( vioGetElementData ( lp, "ammoTyp"..i ) ), false, false )
					end
				end
			end
		end
