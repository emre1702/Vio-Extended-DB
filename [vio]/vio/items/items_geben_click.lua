...
function showItemGiveList()
						...
						if itemCommand[selectedText] == "eat" then
							for i = 1, 3 do
								if foodName[getElementData ( lp, "food"..i )] == selectedText then
									triggerServerEvent ( "giveitem", lp, getElementData ( lp, "curclicked" ), itemCommand[selectedText], i )
									break
								end
							end
						else
							triggerServerEvent ( "giveitem", lp, getElementData ( lp, "curclicked" ), itemCommand[selectedText], nil, tonumber ( guiGetText ( gEdit["itemGiveAmount"] ) ) )
						end
					elseif placeAbleObjects[selectedText] then
						triggerServerEvent ( "giveitem", lp, getElementData ( lp, "curclicked" ), "object" )
					end
				end
			end,
		false )
	end
	local name = getElementData ( lp, "curclicked" )
	if isElement ( getPlayerFromName ( name ) ) then
		...
		guiGridListSetItemText ( grid, row, columnCount, getElementData(lp,"money").."$", false, false )
	else
		hideItemGiveList()
