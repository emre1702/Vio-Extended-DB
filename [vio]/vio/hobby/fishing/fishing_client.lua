...
function beginFishing ()
	...
	if getElementData ( lp, "fishingPole" ) and getElementData ( lp, "fishingHooks" ) > 0 and getElementData ( lp, "fishingWorms" ) > 0 then
...
function fishingShop_hit ( player, dim )
	...
	if player == lp and dim and not getPedOccupiedVehicle ( player ) then
		...
		local hooks = getElementData ( lp, "fishingHooks" )
		local worms = getElementData ( lp, "fishingWorms" )
