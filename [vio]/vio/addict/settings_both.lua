...
function getTotalAddictLevel ( player )
	...
	local points1 = vioGetElementData ( player, "cigarettAddictPoints" )
	local points2 = vioGetElementData ( player, "alcoholAddictPoints" )
	local points3 = vioGetElementData ( player, "drugAddictPoints" )
...
function getCigarettAddictLevel ( player )
	...
	if isElement ( player ) then
		local points = vioGetElementData ( player, "cigarettAddictPoints" )
...
function getAlcoholAddictLevel ( player )
	...
	if isElement ( player ) then
		local points = vioGetElementData ( player, "alcoholAddictPoints" )
...
function getDrugAddictLevel ( player )
	...
	if isElement ( player ) then
		local points = vioGetElementData ( player, "drugAddictPoints" )
