...
function showStreetCleanGUI_func ()
	...
	if gWindow["cleanJob"] then
		...
		gLabel[3] = guiCreateLabel(11,75,190,60,"Bewege dich ueber das Gelaende\nund sammle den Muell ein -\npro eingesammeltem Teil Muell\nbekommst du 3 $ Bezahlung.",false,gWindow["cleanJob"])
		...
		gLabel[5] = guiCreateLabel(24,310,180,44,"Fahr mit dem Muellwagen durch\ndie Stadt und leere die Tonnen.\nPro Tonne erhaelst du 80 $.",false,gWindow["cleanJob"])
		...
		gButton[3] = guiCreateButton(53,356,111,41,"Als Muellwagen-\nfahrer arbeiten",false,gWindow["cleanJob"])
		...
		addEventHandler ( "onClientGUIClick", gButton[2], 
			function()
				if getElementData ( lp, "streetCleanPoints" ) >= 50 then
					guiSetVisible ( gWindow["cleanJob"], false )
					showCursor ( false )
					triggerServerEvent ( "cancel_gui_server", lp )
		...
		addEventHandler ( "onClientGUIClick", gButton[3], 
			function()
				if getElementData ( lp, "streetCleanPoints" ) >= 250 then
					guiSetVisible ( gWindow["cleanJob"], false )
					showCursor ( false )
					triggerServerEvent ( "cancel_gui_server", lp )
					...
					infobox ( "\nDu benoetigst mind.\n250 Jobpunkte, um\nals Muellwagenfahrer\narbeiten zu koennen.", 5000, 125, 0, 0 )
				end
			end,
		false )
	end
	guiSetText ( gLabel[2], tostring ( getElementData ( lp, "streetCleanPoints" ) ) )
end
