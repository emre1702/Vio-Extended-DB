...
bikerGate = createObject ( 976, -2193.2431640625, -2348.7509765625, 29.749998092651, 0, 0, 230 )

bikerGateMoving = false
...
function mv_func ( player )
	...
	if isBiker ( player ) or isOnDuty(player) then
		...
		if getDistanceBetweenPoints3D ( -2197.197265625, -2352.8564453125, 30.625, getElementPosition ( player ) ) < 10 then
		
			if not bikerGateMoving then
				bikerGateMoving = true
				if bikerGateMoved == false then
					moveObject ( bikerGate, 3000, -2193.2431640625, -2348.7509765625, 29.749998092651, 0, 0, 0 )
					setTimer (
						function ()
							bikerGateMoving = false
						end,
					...
					moveObject ( bikerGate, 3000, -2189.0187988281, -2343.7065429688, 29.749998092651, 0, 0, 0 )
					setTimer (
						function ()
							bikerGateMoving = false
						end,
