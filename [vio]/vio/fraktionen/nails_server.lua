		...
		function ( element, dim )
			...
			if dim and nail_tut[source] then
				...
				if getElementType( element ) == "vehicle" then
					...
					if ( nailavai == true ) and isElement(trigger) then outputChatBox ( "Benutze /nailload, um die Nagelbaender aufzuladen. Du musst aber in einem Polizeiwagen sitzen.", trigger, 0, 100, 0 ) end
													
				elseif getElementType( element ) == "player" and getPedOccupiedVehicleSeat( element ) ~= 0 then
				
					if nailavai == true and isElement(element) then outputChatBox ( "Benutze /nailload, um die Nagelbaender aufzuladen. Du musst aber in einem Polizeiwagen sitzen.", element, 0, 100, 0 ) end
			...
			if not veh then
				...
				if nail_cars[getElementModel(veh)] or federalVehicles[veh] then
					...
					if loadedNails == false then
						...
						if isElement( player ) then outputChatBox ( "Du hast jetzt "..tostring(maxnail).." Nagelbaender im Wagen.", player, 0, 100, 0 ) end
						if isElement( player ) then outputChatBox ( "Benutze /takenail <Anzahl>, um dir Nagelbaender zu nehmen.", player, 0, 100, 0 ) end
				...
				if not veh then
					...
					if nail_cars[getElementModel(veh)] or federalVehicles[veh] then
						...
						if loadedNails == false then
							...
							if isElement( player ) then outputChatBox ( "Dein Fahrzeug hat keine Nagelbaender!", player, 255, 0, 0 ) end
							...
							if tonumber(arg) <= tonumber(loadedNails) then
								...
								if isElement( player ) then outputChatBox ( "Du hast " .. arg .. " Nagelbaender genommen!", player, 0, 100, 0 ) end
								if isElement( player ) then outputChatBox ( "Benutze /nail, um ein Nagelband zu legen!", player, 0, 100, 0 ) end
								
							else
							
								if isElement( player ) then outputChatBox ( "Dein Fahrzeug hat nicht genug Nagelbaender!", player, 255, 0, 0 ) end
...
addCommandHandler ( "rnails", 
	function ( player, command )
		...
		if ( nail_factions[fraktion] and ( rang >= 5 ) ) or ( admin >= 1 ) then
			...
			----------------------------------------------------------
				...
				sendMSGForFaction ( getPlayerName(player) .. " hat alle Nagelbaender entfernt.", 1, 0, 100, 0 )
				sendMSGForFaction ( getPlayerName(player) .. " hat alle Nagelbaender entfernt.", 6, 0, 100, 0 )
				sendMSGForFaction ( getPlayerName(player) .. " hat alle Nagelbaender entfernt.", 8, 0, 100, 0 )
			----------------------------------------------------------
...
addCommandHandler ( "setnailmax", 
	function ( player, command, arg )
		...
		if admin >= 3 and arg then
			...
			if isElement( player ) then outputChatBox ( "Du hast die maximalen Nagelbaender pro Wagen auf "..arg.." gesetzt!", player, 0, 100, 0 ) end
