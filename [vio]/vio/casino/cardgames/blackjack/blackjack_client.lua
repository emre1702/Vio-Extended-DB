...
for key, index in pairs ( blackJackTables["x1"] ) do
	...
	local blackJackMarker = createMarker ( x3, y3, z3, "cylinder", 1, 125, 0, 0, 150 )
	setElementData ( blackJackMarker, "cx", x2 )
	setElementData ( blackJackMarker, "cy", y2 )
	setElementData ( blackJackMarker, "cz", z2 )
	setElementData ( blackJackMarker, "id", key )
	setElementInterior ( blackJackMarker, int )
	addEventHandler ( "onClientMarkerHit", blackJackMarker,
		function ( hit )
			if hit == lp then
				if getElementData ( lp, "casinoChips" ) > 0 then
					if not blackJackStarted then
						local x1, y1, z1 = getElementData ( source, "cx" ), getElementData ( source, "cy" ), getElementData ( source, "cz" )
...
function blackJackScrollBarChanged ()
	...
	local money = math.floor ( getElementData ( lp, "casinoChips" ) / 100 * guiScrollBarGetScrollPosition ( gScroll["blackJack"] ) + 0.5 )
...
function blackJackEditChanged ()
	...
	if scrollBarChanged then
		...
		if curTotalEditChips ~= guiGetText ( gEdit["blackJack"] ) then
			...
			if money then
				if money <= getElementData ( lp, "casinoChips" ) then
					money = math.floor ( money + 0.5 )
					guiSetText ( gEdit["blackJack"], money )
					curTotalEditChips = money
					money = money / getElementData ( lp, "casinoChips" ) * 100
					guiScrollBarSetScrollPosition ( gScroll["blackJack"], money )
				else
					guiSetText ( gEdit["blackJack"], getElementData ( lp, "casinoChips" ) )
				end
			else
