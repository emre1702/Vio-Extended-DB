...
for i, index in pairs ( roulettTables["x"] ) do
		...
		function ( hit )
			if hit == lp then
				if not roulettStarted then
					...
					local x, y, z = getElementData ( source, "tx" ), getElementData ( source, "ty" ), getElementData ( source, "tz" )
	...
	setElementData ( roulettMarker, "tx", x )
	setElementData ( roulettMarker, "ty", y )
	setElementData ( roulettMarker, "tz", z )
end
