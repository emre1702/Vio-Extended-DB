...
sprunkAutomats = {}
 ...
 sprunkAutomats["z"] = {}
 i = 1
 sprunkAutomats["x"][i], sprunkAutomats["y"][i], sprunkAutomats["z"][i] = -1980.7857666016, 142.66093444824, 27.336725234985
 i = 2
...
for i, index in pairs ( sprunkAutomats["x"] ) do
	...
	addEventHandler ( "onClientColShapeHit", shape,
		function ( hit, dim )
			if hit == lp then
				if dim then
					curSprunkShape = getElementData ( source, "i" )
				end
			end
		end
	)
...
function sprunkTutBubbles ()
	i = 1
	while isElement ( _G["SprunkBox"..i] ) do
		local x, y, z = getElementPosition ( _G["SprunkBox"..i] )
		createTutorialBubble ( x, y, z, "An diesem Automaten\nkannst du deine\nGesundheit wieder\nauffrischen und deinen\nHunger stillen.\nDruecke die Ein-\nsteigen-Taste.", 5, 200, 200, 0 )
		i = i + 1
	end
end
setTimer ( sprunkTutBubbles, 20000, 1 )
	...
	if not getPedOccupiedVehicle ( lp ) then
		if curSprunkShape > 0 then
			...
			if not isUsingSprunkAutomat then
				...
				if dist < 3 then
					if getElementData ( lp, "money" ) >= 1 then
...
function killAnimation ( player )
	...
	eatSomething_func ( 25 )
	triggerServerEvent ( "sprunkAutomatUse", getRootElement(), player )
end
