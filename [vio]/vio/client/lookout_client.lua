...
for key, index in pairs ( lookouts["x"] ) do
	...
	_G["lookout"..key] = createObject ( lookouts["model"], x, y, z, 0, 0, r )
	x, y = getPointFromDistanceRotation ( x, y, dist, lookoutMarkerRot + r )
	local lookoutMarker = createMarker ( x, y, z - lookoutMarkerZDist, "cylinder", 1, 255, 0, 0, 150 )
	_G["lookoutMarker"..key] = lookoutMarker
	setElementData ( lookoutMarker, "id", key )
	addEventHandler ( "onClientMarkerHit", lookoutMarker,
		function ( hit )
			if hit == lp then
				nextLookout = getElementData ( source, "id" )
...
function useViewPoint ()
	...
	local next = nextLookout
	local lookout = _G["lookout"..next]
	if lookout then
		...
		if getDistanceBetweenPoints3D ( dx1, dy1, dz1, dx2, dy2, dz2 ) <= 5 then
			triggerServerEvent ( "lookoutFound", lp, next )
			
			local x, y, z = getElementPosition ( lookout )
			local rx, ry, rz = getElementRotation ( lookout )
			local r = rz
			
			local x1, y1 = getPointFromDistanceRotation ( x, y, lookDistance, r )
			local x2, y2 = getPointFromDistanceRotation ( x1, y1, 50, r )
			...
			setElementAlpha ( _G["lookoutMarker"..next], 0 )
...
function hideLookoutMarkers_func ( dataString )
	...
	for i = 1, 10 do
		...
		if tok == 1 then
			setElementAlpha ( _G["lookoutMarker"..i], 0 )
		end
	end
end
