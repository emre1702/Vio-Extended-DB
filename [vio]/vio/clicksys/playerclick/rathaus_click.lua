...
function rathausClick ()
	if gWindow["rathausbg"] then
		...
		elseif selectedText == "LKW-Fuehrerschein" then
			license = "lkw"
			refreshCityhallTexts()
		elseif selectedText == "Personalausweiss" then
			license = "perso"
			refreshCityhallTexts()
...
function refreshLicenses()
	...
	guiGridListSetItemText( gGrid["Licenses"], row, gColumn["cityhallPreis"], "1.495 $", false, false )
	...
	local row = guiGridListAddRow ( gGrid["Licenses"] )
	guiGridListSetItemText( gGrid["Licenses"], row, gColumn["cityhallLicense"], "Personalausweiss", false, false )
...
function refreshCityhallTexts()
	...
	elseif license == "perso" then
		guiSetText ( gLabel["cityhalInfotext2"], "Personalausweiss" )
