			...
			if getElementType ( hit ) == "player" then
				if not getPedOccupiedVehicle ( hit ) then
					if vioGetElementData ( hit, "gunlicense" ) == 1 then
						...
						local x, y, z, r = vioGetElementData ( source, "x" ), vioGetElementData ( source, "y" ), vioGetElementData ( source, "z" ), vioGetElementData ( source, "r" )
						vioSetElementData ( hit, "shootingRangeX", x )
						vioSetElementData ( hit, "shootingRangeY", y )
						vioSetElementData ( hit, "shootingRangeZ", z )
						vioSetElementData ( hit, "shootingRangeR", r )
					else
...
function startShootingRanch_func ( gun )
	...
	vioSetElementData ( player, "shootingRanchGun", id )
end
...
function endShootingRanchTest_func ( percent, hit )
	...
	if hit >= 65 and tonumber ( percent ) >= 80 and vioGetElementData ( player, "shootingRanchGun" ) == 24 then
	...
	vioSetElementData ( player, "shootingRanchGun", nil )
	
	local x, y, z, r = vioGetElementData ( player, "shootingRangeX" ), vioGetElementData ( player, "shootingRangeY" ), vioGetElementData ( player, "shootingRangeZ" ), vioGetElementData ( player, "shootingRangeR" )
	setPedRotation ( player, r )
	setElementPosition ( player, x, y, z )
	
	takeWeapon ( player, vioGetElementData ( player, "shootingRanchGun" ) )
end
