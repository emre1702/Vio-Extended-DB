...
function playerFriendlistJoin ()
	...
	if _G["friend"..getPlayerName(source)] and getElementData ( localPlayer, "premium" ) then
		setElementData ( source, "isonfriendlist", true, false )
		showFriendlistCurState ( source, "online" )
	else
		setElementData ( source, "isonfriendlist", false, false )
	end
end
addEventHandler( "onClientPlayerJoin", getRootElement(), playerFriendlistJoin )

function playerFriendlistQuit ()

	if getElementData ( source, "isonfriendlist" ) and getElementData ( localPlayer, "premium" ) then
		showFriendlistCurState ( source, "offline" )
	end
end
addEventHandler( "onClientPlayerQuit", getRootElement(), playerFriendlistQuit )

function loadFriendlist_func ()

	local friendlist = xmlLoadFile ( ":vio/friendlist/friendlist.xml" )
	if friendlist then
...
function fillFriendlist()
	...
	if gGrid["friendlistNames"] then
		...
		if not friendlist then
			local friendlist = xmlCreateFile ( ":vio/friendlist/friendlist.xml", "friendlist" )
